...
class InitCommand extends models.Command {
                    ...
                    Promise.all(files.map(file => {
                        ...
                        return new Promise((resolve1, reject1) => {
                            ejs__default["default"].renderFile(filePath, projectInfo, {}, (err, result) => {
                                if (err) {
                                    utils.log.verbose('ejsRender', err.toString());
                                    reject1(err);
                                }
                                else {
    ...
    prepare() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const spinner = utils.spinnerStart('从远程仓库获取模版');
            ...
            const projectNamePrompt = {
                ...
                validate: function (v) {
                    return new Promise((resolve, reject) => {
                        setTimeout(() => {
                            if (!isValidName(v)) {
                                reject(`合法${title}名称需满足：1.首字符必需为英文字母或@符号; 2.尾字符必需为英文或数字，不能为字符;3.字符仅允许'-_'`);
                                return;
                            }
                            resolve(true);
                        }, 0);
